...
我有 ADHD，桌面上放一只会盯着我的小东西，比任何待办 app 都管用——它不催、不弹通知，
就是安安静静坐在角落里陪着，我反而坐得住了。找不到顺手的，就自己写了一只。
...
面板本身：拖标题栏移动，点右上角 `—` 收起成贴纸大小，**双击贴纸**展开（单击是拖动，所以挪位置不会误触）。**右键**弹菜单：贴边收起 / 收起 / 退出。

三种大小，一种比一种小，但**没有一种会让它彻底消失**——这是故意的。之前做过一个「隐藏」把整个窗口 `orderOut` 藏起来，结果有一次卡在隐藏态，怎么都找不回来，那种设计等于把宠物弄丢了。现在最小也看得见：

- **面板**：完整的清单 + 计时 + Clawd。
- **贴纸**（右上角 `—`）：112×76 的小贴纸，Clawd 加一行状态字，还留在桌面上。
- **贴边**（右键→贴边收起）：缩成屏幕右缘一个 26 像素的小舌头，只露 Clawd 的迷你脸；计时在跑的话舌头左沿有一条进度线。**点一下**就回到收起前的位置。

**收起和贴边，计时都照走**——不会藏着把这一段白计了。到点它会自己冒出来告诉你。

不管哪种大小，Clawd 都不是死图：待命时会眨眼、偶尔往旁边瞟一眼、犯困了低头冒 Zzz；专注时抱着电脑陪你；歇会儿就真端起杯子。杯子、Zzz 这些道具在小尺寸下会自动挪到身侧，不会出界。

万一桌面太乱真找不着，`clawd.sh find` 会把它展开、放回左上角、置顶。
